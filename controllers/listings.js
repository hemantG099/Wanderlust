...
module.exports.createLisitng = async (req, res, next) =>  {
  ...
  .send()
    let ctg = req.body.listing.category;
    console.log(ctg);
    let url = req.file.path;
    let filename = req.file.filename;
     const newListing = new Listing(req.body.listing);
     newListing.category = ctg;
...
module.exports.filterListing = async (req, res) => {
 ...
 module.exports.searchListing =  async(req, res) => {
        let query = req.query.city;   // we expect ?city=Delhi
        if (!query) {
            req.flash("error", "Please enter a city");
            return res.redirect("/listings");
        }

        const listings = await Listing.find({
            location: { $regex: query, $options: "i" }   // case-insensitive
        });
         if (listings.length === 0) {
            req.flash("error", `No listings found in "${query}"`);
            return res.redirect("/listings");   // back to all listings
        }
